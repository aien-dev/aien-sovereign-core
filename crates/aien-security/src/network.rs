...
impl NetworkPurpose {
    ...
    pub fn is_permitted(&self) -> bool {
        !matches!(self, NetworkPurpose::Telemetry)
    }
    ...
    pub fn as_str(&self) -> &'static str {
        match self {
            ...
            NetworkPurpose::Telemetry => "Telemetry",
        }
    }
}
...
pub fn enforce_network_purpose(intent: &NetworkIntent) -> Result<(), NetworkSecurityError> {
    if intent.purpose == NetworkPurpose::Telemetry {
        tracing::error!(
            ...
            "BLOCKED: Unsolicited outbound telemetry rejected by sovereign network policy"
        );
        return Err(NetworkSecurityError::TelemetryForbidden);
    }
